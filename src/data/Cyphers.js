...
const formFactors = [
  ...
  [68, 'Glass Panel'],
  [69, 'Device that splits into two parts when activated, one with a glass screen'],
];
...
const cyphers = [
  ...
  new Cypher('Poison (Mind-Controlling)', true, 2, [formMap.get(3), formMap.get(4), formMap.get(5), formMap.get(54), formMap.get(55), formMap.get(56)], 'The victim must carry out a specific action in response to a specific trigger', poisonMindControlling),
  ...
  new Cypher('Rejuvenator', true, 2, [formMap.get(3), formMap.get(4), formMap.get(5), formMap.get(37)], 'Substance restores a number of points equal to the cypher’s level to one random Pool. Roll percentile', rejuvenator),
  new Cypher('Remote Viewer', true, 2, [formMap.get(69)], 'For one hour, the glass screen on one part shows everything going on in the vicinity of the other part, regardless of the distance between the two parts.'),
